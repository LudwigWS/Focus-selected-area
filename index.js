...
(function() {
    GM_registerMenuCommand('Focus selected ', function() {

        var myDomOutline = DomOutline({ onClick: focusSelectedArea});

        // Start outline:
        myDomOutline.start();

    }, 'f'); // f is keyboard shortcut.
    ...
    function deleteSiblings(ele) {
        var i, parentEle = ele.parentNode, eles = parentEle.children;
        for (i = eles.length-1; i >= 0; i--) {
            if (eles[i] !== ele) {
                parentEle.removeChild(eles[i]);
            }
        }
    }

    // Lib: https://github.com/andrewchilds/jQuery.DomOutline
